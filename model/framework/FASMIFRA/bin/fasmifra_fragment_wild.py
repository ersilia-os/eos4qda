...
def find_cuttable_bonds(mol, debug=False):
    ...
    for b in mol.GetBonds():
        if (
            (b.GetBondType() == rdkit.Chem.rdchem.BondType.SINGLE)
            and (not b.IsInRing())
            and (b.GetStereo() == rdkit.Chem.rdchem.BondStereo.STEREONONE)
            and (b.HasProp("protected") == 0)
        ):  # HasProp returns an int... :(
            res.append(b)
    return res
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", metavar="input.smi", dest="input_fn", help="molecules input file"
    )
    parser.add_argument(
        "-o", metavar="output.smi", dest="output_fn", help="fragments output file"
    )
    ...
    parser.add_argument(
        ...
        default=1,
        type=int,
        help="number of fragmentation passes",
    )
    ...
    parser.add_argument(
        ...
        default=150.0,
        type=float,
        help="fragment weight (default=150Da)",
    )
    # parse CLI
    if len(sys.argv) == 1:
    ...
    for name, mol in mol_supplier:
        for i in range(nb_passes):
            tagged_bonds_smi, parent_name = tag_cut_bonds(
                frag_weight, randomize, seen_types_dict, mol
            )
            if i == 0:
                print("%s\t%s" % (tagged_bonds_smi, parent_name), file=output)
            else:
                print("%s\t%sp%d" % (tagged_bonds_smi, parent_name, i), file=output)
        count += 1
    print("seen_types: %d" % len(seen_types_dict), file=sys.stderr)
    after = time.time()
    dt = after - before
    print("read %d molecules at %.2f mol/s" % (count, count / dt), file=sys.stderr)
    output.close()
