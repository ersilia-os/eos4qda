...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Create fragments dictionary and encode molecules"
    )
    # can also output as bitstrings the input molecules
    parser.add_argument(
        "-i", metavar="input.smi", dest="smi_in_fn", help="input molecules"
    )
    parser.add_argument(
        "-o", metavar="dico.txt", dest="dico_out_fn", help="fragments output dictionary"
    )
    parser.add_argument(
        "-e", metavar="encoded.txt", dest="bits_out_fn", help="output encoded molecules"
    )
    ...
    parser.add_argument(
        ...
        default=-1,
        type=int,
        help="use only N most frequent fragments (default: -1; no-limit)",
    )
    # parse CLI
    if len(sys.argv) == 1:
